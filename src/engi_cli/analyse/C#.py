...
async def parse_trx(test_output_dir):
    if test_output_dir is None:
        raise RuntimeError("Trx Logger output directory undefined")
    trx_files = [f for f in Path(test_output_dir).glob("*.trx") if f.is_file()]
    if len(trx_files) == 0:
        raise RuntimeError(f"no TRX files found in dir {test_output_dir}")
